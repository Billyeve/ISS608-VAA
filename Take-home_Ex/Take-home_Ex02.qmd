...
It was previously identified that 26 out of 55 Planning Areas have no value/data. To maximize space, 12 Planning Areas will be retained and filtered.
...
trellis12 <- arrange(t_pop_pa, desc(t_pop_pa$sum_pop)) %>%
  slice(1:12) %>%
  select(PA)

trellis12_filter <- pop_pa %>%
  filter(pop_pa$PA %in% trellis24$PA)
```

```{r}
PA_Pyr <- ggplot() +
  geom_bar(data = subset(trellis12_filter, 
                         Sex == "Males"), 
           aes(x = AG, 
               y = -sum_pop, 
               fill = PA),
